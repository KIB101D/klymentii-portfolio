...
const NODE_CONFIG = [
  ...
  { id: "experience", float: { x: -16, y: 14, duration: "13s", delay: "1.4s" } },
...
export function Board() {
  ...
  const nodeRefs = useRef<Array<HTMLButtonElement | null>>([null, null, null, null]);
  ...
  const [drawnCurves, setDrawnCurves] = useState<boolean[]>([false, false, false]);
  ...
  useEffect(() => {
    const timers = [
      ...
      window.setTimeout(() => setVisibleNodes((v) => [true, v[1], v[2], v[3]]), 180),
      window.setTimeout(() => setDrawnCurves((v) => [true, v[1], v[2]]), 380),
      window.setTimeout(() => setVisibleNodes((v) => [v[0], true, v[2], v[3]]), 620),
      window.setTimeout(() => setDrawnCurves((v) => [v[0], true, v[2]]), 820),
      window.setTimeout(() => setVisibleNodes((v) => [v[0], v[1], true, v[3]]), 1060),
      window.setTimeout(() => setDrawnCurves((v) => [v[0], v[1], true]), 1260),
      window.setTimeout(() => setVisibleNodes((v) => [v[0], v[1], v[2], true]), 1500),
    ];
  ...
  function toggleNode(nodeKey: NodeKey) {
    if (isDesktop()) {
      setOpenPopovers((current) => {
        if (current.includes(nodeKey)) return current.filter((key) => key !== nodeKey);
  ...
  return (
    <div ref={boardRef} className={`board ${activeMobile ? "has-open-panel" : ""}`}>
      <div ref={boardBgRef} className="board-bg" />
      <div className="board-vignette" />

      <CurveLayer register={(index, element) => { curveRefs.current[index] = element; }} drawn={drawnCurves} />

      <div className={`board-header ${headerVisible ? "is-visible" : ""}`}>
        <span className="dot">◆</span> {t.board.header}
      </div>

      <nav className={`quicklinks ${quicklinksVisible ? "is-visible" : ""}`} aria-label="Quick links">
        <a href="https://github.com/KIB101D" target="_blank" rel="noopener noreferrer">{t.board.quickGitHub}</a>
        ...
        <div className="locale-switcher" aria-label="Language">
          {(["en", "uk", "pl"] as const).map((item) => (
            <button key={item} className={locale === item ? "is-active" : ""} type="button" onClick={() => setLocale(item)} aria-pressed={locale === item}>{item.toUpperCase()}</button>
          ))}
        </div>
      </nav>

      <div ref={coordsRef} className={`coords ${coordsVisible ? "is-visible" : ""}`}>x · y</div>
      <div className={`hint ${hintVisible ? "is-visible" : ""}`}>{t.board.hint}</div>

      {NODE_CONFIG.map((node, index) => (
        <Node key={node.id} id={node.id} label={t.board.nodes[node.id]} float={node.float} visible={visibleNodes[index]} active={activeMobile === node.id || openPopovers.includes(node.id)} onClick={() => toggleNode(node.id)} register={(element) => { nodeRefs.current[index] = element; }} />
      ))}
      ...
      {openPopovers.map((nodeKey, index) => {
        ...
        return <Popover key={nodeKey} nodeKey={nodeKey} anchor={anchor} zIndex={21 + index + (popoverZ - 20)} onClose={() => setOpenPopovers((current) => current.filter((key) => key !== nodeKey))} />;
      })}

      <div className={`backdrop ${activeMobile ? "is-open" : ""}`} onClick={() => setActiveMobile(null)} aria-hidden="true" />
      <aside className={`panel ${activeMobile ? "is-open" : ""}`} aria-hidden={!activeMobile}>
        <button className="panel-close" type="button" aria-label={t.board.close} onClick={() => setActiveMobile(null)}>✕</button>
        <div className="panel-content">{ActiveContent ? <ActiveContent /> : null}</div>
      </aside>
    </div>
  );
}
