...
const NODE_CONFIG = [
  {
    id: "about",
    label: "About",
    float: { x: 14, y: -18, duration: "11s", delay: "0s" },
  },
  {
    id: "experience",
    label: "Experience",
...
export function Board() {
  ...
  useMousePosition(boardBgRef, coordsRef);
  ...
  function closePopover(nodeKey: NodeKey) {
    setOpenPopovers((current) => current.filter((key) => key !== nodeKey));
  }
  ...
  return (
    ...
    >
      ...
      <div className={`board-header ${headerVisible ? "is-visible" : ""}`}>
        <span className="dot">◆</span> frontend / board
      </div>
      ...
      >
        <a href="https://github.com/KIB101D" target="_blank" rel="noopener noreferrer">
          GitHub ↗
        </a>
        <a href="mailto:boiko.klymentii.ua@gmail.com">Email ↗</a>
      </nav>
      ...
      {NODE_CONFIG.map((node, index) => (
        <Node
          ...
          active={
            activeMobile === node.id || openPopovers.includes(node.id)
          }
